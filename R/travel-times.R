...
#'
#' @param shp Area of interest, can be sf of SpatialPolygonDataFrame object, or character string for specific country (will use malariaAtlas shapefile).
...
#' @param transport_type Either "walking" (default) or "motorized".
#' @param export TRUE/FALSE Should output be saved?
#' @param output_dir String to output folder location.
#'
...
get_friction_surface <- function(
  ...
  if (!transport_type %in% c("walking", "motorized")) stop('transport_type must be either "walking" or "motorized".')

  if (transport_type == "walking") {
    rstr <- "Global friction surface enumerating land-based travel walking-only speed without access to motorized transport for a nominal year 2019"
  }

  if (transport_type == "motorized") {
    rstr <- " Global friction surface enumerating land-based travel speed with access to motorized transport for a nominal year 2019"
  }

  # Classify shapefile
  ## Convert sf object to sf
  if ("sf" %in% class(shp)) {
    shp <- sf::as_Spatial(shp)
  }

  ## If text, download shapefile
  if (is.character(shp)) {
    suppressMessages(shp <- malariaAtlas::getShp(
      country = shp,
      admin_level = "admin0"
    ))
  }

  # Pull surface from MAP
  friction_surface <- malariaAtlas::getRaster(
    surface = rstr,
    shp = shp,
    extent = extent
  )
  ...
  if (export) raster::writeRaster(friction_surface, output_dir)

  return(friction_surface)
}
...
#' @param save_transition_matrix TRUE/FALSE: Save transition matrix object? Useful for large areas.
...
create_travel_surface <- function(
  ...
  fs1 <- raster::crop(friction, new_extent)
